...
RUN passwd -l ai_agent
RUN groupadd developer
RUN usermod -a -G developer ai_agent
COPY ./files/agent_container /app/agent_container
RUN chown -R root:developer /app/agent_container
RUN chmod 755 /app/agent_container
#RUN sudo -u ai_agent bash /app/agent_container/build.sh
...
RUN usermod -a -G developer ${USER_NAME}
#section-end
#section-start set up user home!
